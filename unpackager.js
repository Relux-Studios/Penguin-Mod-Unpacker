...
var unpackage = (function() {
  ...
  const unpackageBinaryBlob = async (data) => {
    ...
    if (projectZip) {
      // The project is a compressed .snail project.
  ...
  const unpackage = async (blob) => {
    ...
    if (packagedZip) {
      // Raw sb2, raw .snail, and zip files generated by TurboWarp Packager have a project.json alongside the assets.
      ...
      if (projectJSON) {
        ...
        const type = sb2Assets > 0 && sb3Assets === 0 ? 'sb2' : 'snail';
